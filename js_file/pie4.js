...
g6= svg6.append("g").attr("transform", "translate(" + (width3+80) / 2 + "," + (height3+80) / 2 + ")");
...
d3.json("./data/yelp_bar.json").then(result => {
...
d3.select(event.currentTarget).select('path')
                ...
                .attr('stroke-width', 5)
                .attr('stroke','black');
d3.select('.card-back text').text(v.data.category);
...
d3.select(event.currentTarget)
    ...
    arc3.append("path")
        .attr("d", path3)
        .attr("fill", function(d) { return color3(d.data.category); })
        .attr('stroke-width','2')
        ;
